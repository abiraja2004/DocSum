...
def findConcept(sents, conFile):
    f = open(conFile)
    lines = f.readlines()
    f.close()
    strList = [l.strip().split("\t") for l in lines]
    print strList[:10]
    weightList = [(t[0], float(t[1])) for t in strList]
    conStrWeight = dict(weightList)
    ...
    for i in range(len(sents)):
        ...
        bigrams = set(bigrams)
        for c in bigrams:
            if c not in conStrWeight:
                continue
            if c not in conMap:
                conMap[c] = len(conMap)
                conID = conMap[c]
                conWeight[conID] = conStrWeight[c]
            conID = conMap[c]
...
def modelling(conWeight, sentCon, lenList, limit):
    ...
    for i in range(sentLen):
        if not sentCon[i]:
            sm = "x_"+str(i)+" = LpVariable(\"x_"+str(i)+"\", 0, 0, \"Integer\")"
        else:
