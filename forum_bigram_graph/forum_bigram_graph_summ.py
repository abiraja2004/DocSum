...
def summarize(origFile, sentFile, limit=100, outDir="./", alpha=1, beta=1, gamma=1, diff=0.000000001, damping=0.85, maxIter=200):
    sentG = bg.sentGroup(origFile, sentFile)
    wordG = bg.wordGraph(sentG)
    ...
    f = open(outDir+prefix+".gillick_graph.result","w")
    f.write("\n".join(summary))
    f.close()